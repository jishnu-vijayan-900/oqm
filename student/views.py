...
def start_exam_view(request,pk):
    st = models.Student.objects.get(user_id=request.user.id)
    stw = QMODEL.Course.objects.get(id=pk)

    if QMODEL.Result.objects.filter(student_id=st,exam_id=stw):
        return render(request,'student/student_dashboard.html',{'message': "Already attend this exam"})
    else:
        course = QMODEL.Course.objects.get(id=pk)
        questions = QMODEL.Question.objects.all().filter(course=course)
        if request.method == 'POST':
...
def calculate_marks_view(request):
    if request.COOKIES.get('course_id') is not None:
        ...
        actualmarks = 0
        for q in questions:
            actualmarks = actualmarks + q.marks
        ...
        result.total_marks=actualmarks
        result.save()
...
def view_student_view(request):
    student = models.Student.objects.get(user_id=request.user.id)
    return render(request,'student/admin_view_student.html',{'students':student})
